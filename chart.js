...
class Chart {
  ...
  barChart() {
    ...
    const drawAvg = (context, data, color) => {
      context.fillStyle = color;
      for (let i = 0; i < Object.keys(data.avg).length; i++) {
        context.fillRect(
          46,
          25 + 40 * i,
          Object.values(data.avg)[i] * 2.25, // 1% === 2.25
          20
        );
      }
    };
    const drawMydata = (context, data, color) => {
      context.fillStyle = color;
      for (let i = 0; i < Object.keys(data.myData).length; i++) {
        this.context.fillRect(
          46 + Object.values(data.myData)[i] * 2.25,
          25 + i * 40,
          20,
          20
        );
  ...
  lineChart() {
    ...
    const drawLine = (context, data, color) => {
...
chart1.radarChart();
const chart2 = new Chart(data);
chart2.barChart();
const chart3 = new Chart(data);
chart3.radar5Chart();
